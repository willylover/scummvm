...
void Cast::loadFontMapV4(Common::SeekableReadStreamEndian &stream) {
	...
	for (uint32 i = 0; i < entriesUsed; i++) {
		...
		// Map cast font ID to window manager font ID
		FontMapEntry *entry = new FontMapEntry;
		entry->remapChars = platform != _vm->getPlatform();
		if (platform != _vm->getPlatform() && _fontXPlatformMap.contains(name)) {
...
bool Cast::readFXmpLine(Common::SeekableReadStreamEndian &stream) {
	...
	if (fromFont.empty()) {
		...
		// Retain the direction which converts this cast's source fonts to the
		// platform Director is currently emulating. We only handle one font map
		// per source font (multiple entries occur in Clone Ranger).
